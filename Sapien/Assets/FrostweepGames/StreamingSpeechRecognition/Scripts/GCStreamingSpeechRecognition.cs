...
namespace FrostweepGames.Plugins.GoogleCloud.StreamingSpeechRecognition
{
    public class GCStreamingSpeechRecognition: MonoBehaviour
    {
		...
		private const int StreamingRecognitionTimeLimit = 290; // around 5 minutes. ~ 4.83333334
		...
		private List<float> _currentRecordedSamples;
		...
		private bool StartRecording()
		{
			...
			_currentRecordedSamples = new List<float>();
		...
		private async void WriteDataToStream()
		{
			...
			if (isRecording || (_currentRecordedSamples != null && _currentRecordedSamples.Count > 0))
			{
				...
				{
					samplesChunk = _currentRecordedSamples.GetRange(0, AudioChunkSize);
				...
				{
					byte[] buffer = CustomMicrophone.FloatToByte(samplesChunk.ToArray());

					chunk = ByteString.CopyFrom(buffer, 0, buffer.Length);
...
#pragma warning disable CS1701
				...
				{
					ChannelCredentials = GoogleCredential.FromJson(
						config.googleCredentialLoadFromResources ? Resources.Load<TextAsset>(config.googleCredentialFilePath).text : config.googleCredentialJson)
